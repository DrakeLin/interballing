...
def load_manifest(json_data: dict, conn: pyodbc.Connection) -> Exception:
    ...
    """
    cursor = conn.cursor()
    fieldnamesql = "INSERT INTO IB.players (id, full_name)"
    today = date.today()
    ...
    try:
        ...
        for item in items:
            ...
            valsql += "'" + str(item.get("id")) + "', "
            valsql += "'" + item.get("full_name") + "')"#"', "
            #valsql += "'" + mydate + 
...
def main():
    with open('./all_players.json') as f:
        data = json.load(f)
